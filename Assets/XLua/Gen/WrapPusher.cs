...
namespace XLua
{
    public partial class ObjectTranslator
    {
        ...
        class IniterAdderUnityEngineVector2
        {
			...
			static void Init(LuaEnv luaenv, ObjectTranslator translator)
			{
				...
				translator.RegisterPushAndGetAndUpdate<UiType>(translator.PushUiType, translator.Get, translator.UpdateUiType);
				translator.RegisterPushAndGetAndUpdate<MessageType>(translator.PushMessageType, translator.Get, translator.UpdateMessageType);
        ...
        int UiType_TypeID = -1;
		int UiType_EnumRef = -1;
        
        public void PushUiType(RealStatePtr L, UiType val)
        {
            if (UiType_TypeID == -1)
            {
			    bool is_first;
                UiType_TypeID = getTypeId(L, typeof(UiType), out is_first);
				
				if (UiType_EnumRef == -1)
				{
				    Utils.LoadCSTable(L, typeof(UiType));
				    UiType_EnumRef = LuaAPI.luaL_ref(L, LuaIndexes.LUA_REGISTRYINDEX);
				}
				
            }
			
			if (LuaAPI.xlua_tryget_cachedud(L, (int)val, UiType_EnumRef) == 1)
            {
			    return;
			}
			
            IntPtr buff = LuaAPI.xlua_pushstruct(L, 4, UiType_TypeID);
            if (!CopyByValue.Pack(buff, 0, (int)val))
            {
                throw new Exception("pack fail fail for UiType ,value="+val);
            }
			
			LuaAPI.lua_getref(L, UiType_EnumRef);
        ...
        public void Get(RealStatePtr L, int index, out UiType val)
        {
		    LuaTypes type = LuaAPI.lua_type(L, index);
            if (type == LuaTypes.LUA_TUSERDATA )
            {
			    if (LuaAPI.xlua_gettypeid(L, index) != UiType_TypeID)
				{
				    throw new Exception("invalid userdata for UiType");
				}
                ...
                {
                    throw new Exception("unpack fail for UiType");
                }
				val = (UiType)e;
            ...
            {
                val = (UiType)objectCasters.GetCaster(typeof(UiType))(L, index, null);
            }
        }
		
        public void UpdateUiType(RealStatePtr L, int index, UiType val)
        {
            ...
            if (LuaAPI.lua_type(L, index) == LuaTypes.LUA_TUSERDATA)
            {
                ...
                if (!CopyByValue.Pack(buff, 0,  (int)val))
                {
                    throw new Exception("pack fail for UiType ,value="+val);
                }
            }
        ...
        int MessageType_TypeID = -1;
		int MessageType_EnumRef = -1;
        
        public void PushMessageType(RealStatePtr L, MessageType val)
        {
            if (MessageType_TypeID == -1)
            {
			    bool is_first;
                MessageType_TypeID = getTypeId(L, typeof(MessageType), out is_first);
				
				if (MessageType_EnumRef == -1)
				{
				    Utils.LoadCSTable(L, typeof(MessageType));
				    MessageType_EnumRef = LuaAPI.luaL_ref(L, LuaIndexes.LUA_REGISTRYINDEX);
				}
				
            }
			
			if (LuaAPI.xlua_tryget_cachedud(L, (int)val, MessageType_EnumRef) == 1)
            {
			    return;
			}
			
            IntPtr buff = LuaAPI.xlua_pushstruct(L, 4, MessageType_TypeID);
            if (!CopyByValue.Pack(buff, 0, (int)val))
            {
                throw new Exception("pack fail fail for MessageType ,value="+val);
            }
			
			LuaAPI.lua_getref(L, MessageType_EnumRef);
			LuaAPI.lua_pushvalue(L, -2);
			LuaAPI.xlua_rawseti(L, -2, (int)val);
			LuaAPI.lua_pop(L, 1);
        ...
        public void Get(RealStatePtr L, int index, out MessageType val)
        {
		    LuaTypes type = LuaAPI.lua_type(L, index);
            if (type == LuaTypes.LUA_TUSERDATA )
            {
			    if (LuaAPI.xlua_gettypeid(L, index) != MessageType_TypeID)
				{
				    throw new Exception("invalid userdata for MessageType");
				}
                ...
                if (!CopyByValue.UnPack(buff, 0, out e))
                {
                    throw new Exception("unpack fail for MessageType");
                }
				val = (MessageType)e;
            ...
            {
                val = (MessageType)objectCasters.GetCaster(typeof(MessageType))(L, index, null);
            }
        }
		
        public void UpdateMessageType(RealStatePtr L, int index, MessageType val)
        {
            ...
            if (LuaAPI.lua_type(L, index) == LuaTypes.LUA_TUSERDATA)
            {
                ...
                if (!CopyByValue.Pack(buff, 0,  (int)val))
                {
                    throw new Exception("pack fail for MessageType ,value="+val);
                }
            }
	...
	public partial class StaticLuaCallbacks
    {
	    internal static bool __tryArrayGet(Type type, RealStatePtr L, ObjectTranslator translator, object obj, int index)
		{
			...
			else if (type == typeof(UiType[]))
			{
			    UiType[] array = obj as UiType[];
				translator.PushUiType(L, array[index]);
				return true;
			}
			else if (type == typeof(MessageType[]))
			{
			    MessageType[] array = obj as MessageType[];
				translator.PushMessageType(L, array[index]);
				return true;
			}
