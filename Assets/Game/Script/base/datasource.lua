...
function DataSource()
	local data_source_class={}
	function data_source_class.New()

	 	local obj=binding.bindable()
	 	for k,v in pairs(data_source_class) do
	 		if k ~= "New" then
	 			obj[k]=v
	 		end
	 	end
	 	obj.Bind=function(self,key,func)
            func(self[key],self[key])
            return binding.watch(self, key, func)
	 	end
        obj.UnBind=function(self,key,tag)
            binding.unwatch(self,key,tag)
        end
	 	obj:__init()
	 	return obj
	end
